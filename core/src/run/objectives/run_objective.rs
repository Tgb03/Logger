...
impl<'a> TryFrom<&'a str> for RunObjective {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        if split.next_if_eq(&"sec").is_some() {
            obj.secondary = true;
        };
        if split.next_if_eq(&"ovrl").is_some() {
            obj.overload = true;
        };
        if split.next_if_eq(&"glitch").is_some() {
            obj.glitched = true;
        };
        if split.next_if_eq(&"edrop").is_some() {
            obj.early_drop = true;
        };
...
mod tests {
    ...
    pub fn test_objectives_grr() {
        ...
        assert_eq!(
            TryInto::<RunObjective>::try_into("R1B1_2_sec.save"),
            Ok(run_obj.clone())
        );
        ...
        assert_eq!(
            TryInto::<RunObjective>::try_into("R1B1_2_sec_ovrl.save"),
            Ok(run_obj.clone())
        );

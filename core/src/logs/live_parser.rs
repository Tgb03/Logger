...
use super::{folder_watcher::FolderWatcher, token::Token};
...
pub struct LiveParser {
    ...
    shutdown_watcher: Option<Sender<()>>,
}
...
impl LiveParser {
    ...
    pub fn start_watcher(&mut self, folder_path: PathBuf) {
        let (recv, sender) = FolderWatcher::new_watcher(folder_path);

        self.folder_watcher = Some(recv);
        self.shutdown_watcher = Some(sender);
    }

    pub fn stop_watcher(&mut self) {
        self.folder_watcher = None;
        self.shutdown_watcher.as_mut().map(|v| v.send(()));

...
use super::{
  ...
  game_run_objective::GameRunObjective, 
...
impl Display for GameObjective {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "{}_{}_{}.rsave", self.rundown, self.objective, self.player_count)
  }
}
...
impl<'a> TryFrom<&'a str> for GameObjective {
  ...
  fn try_from(value: &'a str) -> Result<Self, Self::Error> {
    ...
    obj.player_count = split.next().unwrap_or_default().parse::<u8>().unwrap_or_default();
...
impl Into<Vec<RunObjective>> for &GameObjective {
  fn into(self) -> Vec<RunObjective> {
    ...
    for (level, objective) in levels.iter().zip(objectives) {
      ...
      if objective.0 { obj_data = obj_data.with_secondary(true) }
      if objective.1 { obj_data = obj_data.with_overload(true) }
...
#[cfg(test)]
mod tests {
  use crate::run::objectives::{game_objective::GameObjective, game_run_objective::GameRunObjective, game_run_rundown::GameRunRundown};
  ...
  pub fn test_objectives() {
    ...
    assert_eq!(TryInto::<GameObjective>::try_into("Rundown1_100%_2.rsave"), Ok(run_obj.clone()));
    assert_eq!(run_obj.to_string(), "Rundown1_100%_2.rsave");

    run_obj.rundown = GameRunRundown::Rundown3;

    assert_eq!(TryInto::<GameObjective>::try_into("Rundown3_100%_2.rsave"), Ok(run_obj.clone()));

...
impl CodeGuess {
    ...
    pub fn new(settings_window: &SettingsWindow) -> Self {
        Self {
            selected: String::new(),
            valid: [true; 733],
            max_lines: settings_window.get::<i32>("code_guess_line_count").unwrap_or(3) as usize,
            codes_per_line: settings_window.get::<i32>("code_guess_word_count").unwrap_or(3) as usize,
        }
    }
...
impl Render for CodeGuess {
    ...
    fn render(&mut self, ui: &mut egui::Ui) -> Self::Response {
        ...
        while counter < 733 && found < self.max_lines * self.codes_per_line {
            ui.horizontal(|ui | {
                while counter < 733 && found < self.max_lines * self.codes_per_line && in_line_counter < self.codes_per_line {
                    if self.valid[counter] {
                        ui.label(
                            unsafe {
                                std::str::from_utf8_unchecked(&ALL_CODES[(counter * 4)..(counter * 4 + 4)])
                            }
                        );

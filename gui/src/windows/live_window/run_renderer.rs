...
impl RunRender {
            ...
            (Some(vd), Some(c_time)) => {
                vd.push_back(match total.cmp(&c_time) {
                    std::cmp::Ordering::Less => (
                        (c_time - total).to_string_no_hours(),
                        Color32::GREEN,
                    ),
                    std::cmp::Ordering::Equal => ("00:00.000".to_string(), Color32::WHITE),
                    std::cmp::Ordering::Greater => (
                        (total - c_time).to_string_no_hours(),
                        Color32::RED,
                    ),
                });
            },
        ...
        match (&mut self.compared_best, split_time, split_compare) {
            (Some(vd), Some(time), Some(c_time)) => {
                vd.push_back(match time.cmp(&c_time) {
                    std::cmp::Ordering::Less => (
                        (c_time - time).to_string_no_hours(),
                        Color32::GREEN
                    ),
                    std::cmp::Ordering::Equal => ("00:00.000".to_string(), Color32::WHITE),
                    std::cmp::Ordering::Greater => (
                        (time - c_time).to_string_no_hours(),
                        Color32::RED,
                    ),
                });
            },
    ...
    fn get_time_wr(&mut self, split: &impl Split, save_manager: &SaveManager) -> Option<Time> {
        self.wr_compared_time += save_manager.get_best_run(&self.objective_str)?
    ...
    fn get_time_split_comparison(&mut self, split: &impl Split, save_manager: &SaveManager) -> Option<Time> {
        save_manager.get_best_split_with_merge(&self.objective_str, split.get_name())
            .cloned()
    }
        ...
        while let Some(r) = self.continous_parser.try_recv() {
            match r {
                RunGeneratorResult::GameStarted(level_descriptor, player_count) => {
                    ...
                    self.run_render = RunRender::new(reader.override_obj(obj).to_string(), settings);
                },
